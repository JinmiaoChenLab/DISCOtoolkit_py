...
def CELLiD_cluster(
    ...
    if n_predict is not None:
        if n_predict > 3:
            logging.info(
                "Any value of n_predict that exceeds 3 will be automatically adjusted to 3."
            )
            n_predict = 3
    ...
    if atlas is not None:
        select_ref = [
            index
            for index, each in enumerate(ref_data.columns)
            if each.split("--")[1] in list(atlas)
        ]
    ...
    if len(genes) <= 2000:
        logging.error(
            "Less than 2000 genes are shared between the input data and the reference dataset!"
        )

    # give a warning to the user
    if len(genes) <= 5000:
        logging.warning(
            "The input data and reference dataset have a limited number of overlapping genes, which may potentially impact the accuracy of the CELLiD."
        )
    ...
    # predicted_cell = pd.DataFrame(
    #     generate_correlation_map(ref_data.to_numpy().T, rna.to_numpy().T)
    # )
    predicted_cell = rna.apply(
        each_ref_correlation, result_type="expand", axis=0, ref_data=ref_data
    )
    ...
    def second_correlation(i, ref_data, rna, ct):
        ref = ref_data.iloc[:, list(ct[str(i)])].copy()
        g = set(
            ref_deg.iloc[check_in_list(ref_deg["group"], ref.columns)].copy()["gene"]
        )
        g = set.intersection(set(ref.index), g)
        ref = ref.loc[list(g)].copy()
        # print(rna.loc[list(g), str(i)])
        input = rna.loc[list(g), str(i)].copy()
        # predict = ref.apply(lambda i: np.round(stats.spearmanr(pd.to_numeric(i), pd.to_numeric(input), nan_policy='omit')[0], 3))
        predict = ref.apply(
            lambda i: np.round(
                pd.Series(i).corr(pd.Series(input), method="spearman"), 3
            )
        )  # trying with pandas correlation
    ...
    predicted_cell = Parallel(n_jobs=ncores, batch_size=32, verbose=2)(
        delayed(second_correlation)(int(y), ref_data, rna, ct)
        for y in range(rna.shape[1])
    )
    ...
    pivot = pd.pivot_table(
        predicted_cell,
        index="input_index",
        columns=predicted_cell.groupby("input_index").cumcount() + 1,
        values=["cell_type", "atlas", "score"],
        aggfunc="first",
    )
...
def CELLiD_enrichment(
    ...
    input["gene"] = input[
        "gene"
    ].str.upper()  # convert all the gene to upper string as the reference gene name is in uppercase
    ...
    if input_shape == 2:
        ...
        input = input.loc[
            list(set(reference["gene"]) & set(input["gene"]))
        ]  # only get the common genes for comparison
    ...
    def process_unique_name(
        reference_filter, input, atlas_dfs, input_shape, atlas_dict
    ):
        ...
        reference_filter = reference_filter.set_index(
            ["gene"], drop=False
        )  # set gene as the index
        # reference_filter["gene"] = reference_filter.index
        input_filter = input.loc[
            list(set(reference_full["gene"]) & set(input["gene"]))
        ]  # subset the input genes
        ...
        if input_shape == 2:
            a = (
                reference_filter.loc[
                    np.intersect1d(reference_filter["gene"], (input_filter["gene"]))
                ].iloc[:, 0]
                * input_filter.loc[
                    np.intersect1d(input_filter["gene"], reference_filter["gene"]), "fc"
                ]
            ).sum() + 1
            b = (
                input_filter.loc[
                    np.setdiff1d(input_filter["gene"], reference_filter["gene"]), "fc"
                ].sum()
                + 1
            )
            c = (
                reference_filter.loc[
                    np.setdiff1d(reference_filter["gene"], input_filter["gene"])
                ]
                .iloc[:, 0]
                .sum()
                + 1
            )
            d = len(set(reference_full["gene"])) - len(
                set(reference_filter["gene"]).union(set(input_filter["gene"]))
            )
        else:
            ...
            return pd.Series(
                {
                    ...
                    "gene": ",".join(
                        reference_filter.loc[
                            reference_filter["gene"].isin(input_filter["gene"]), "gene"
                        ]
                    ),
                    "background": len(set(reference_full["gene"])),
                    "overlap": len(
                        reference_filter.loc[
                            reference_filter["gene"].isin(input_filter["gene"])
                        ]
                    ),
                    "geneset": len(reference_filter),
                }
            )
        else:
            return None
    ...
    results = Parallel(n_jobs=ncores, verbose=2, batch_size=64)(
        delayed(process_unique_name)(
            unique_name_df, input, atlas_dfs, input_shape, atlas_dict
        )
        for unique_name_df in unique_name_dfs
    )
    ...
    if results:
        ...
        res_df = res_df.sort_values(by=["pval", "or"], ascending=[True, False]).head(
            min(50, len(results))
        )
        return res_df
    else:
